...
const ProfileMobile = ({ route }) => {
  // Use optional chaining and a fallback empty object
  const { ProfileShown, CurrentUserId } = route?.params ?? {};
  ...
  useEffect(() => {
    const getUserId = async () => {
      if (ProfileShown === "NORMALUSER" && CurrentUserId !== userid) {
...
const styles = StyleSheet.create({
  ...
  profileImage: { width: 130, height: 130, borderRadius: 65, borderWidth: 2, borderColor: '#FF406C' },
  nameText: { fontSize: 24, fontWeight: 'bold', color: '#000' },
  usernameText: { fontSize: 14, color: '#86878B', marginVertical: 4 },
  goldCardsContainer: { paddingHorizontal: 12 },
  goldCard: { backgroundColor: '#fff', borderRadius: 14, padding: 12, marginRight: 12, width: width - 48,
    shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.23, shadowRadius: 2.62, elevation: 4 },
  goldHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  goldTitleContainer: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  goldTitle: { fontSize: 20, fontWeight: 'bold' },
  upgradeButton: { backgroundColor: '#FFB800', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 16 },
  upgradeButtonText: { color: '#000', fontWeight: 'bold', fontSize: 14 },
  featuresTitle: { fontSize: 16, fontWeight: 'bold', marginBottom: 12 },
  featureRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  featureText: { fontSize: 14 },
  roleBadge: { position: 'absolute', top: 10, left: 10, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 15, zIndex: 1 },
